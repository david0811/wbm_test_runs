...
my $version =		'23.04.0';	# Version. Note- update version variable below
#
#######################################################################
...
use RIMS qw(get_file_path date_now htm_template read_attrib read_table read_table_hash get_calendar calendar360_day calendar360_DaysInMonth calendar365_DaysInMonth make_date_list check_MT_date set_default prepare_dir crs_to_Obj transform_point);
...

{
 ...
 (my  $file_s	= $file) =~ s/.+:(.+):.+/$1/;
  ...
  open STDERR, ">&NEWERR";			# Required for $band->Read to suppress warnings
    my $pdl	= pdl($band->Read);	# Do not use much overloaded "$band->GetPiddle()", e.g. it does resample
  open STDERR, ">&OLDERR";
		...
		### Calculate glacier melt scaling factor due to difference resolutions of source and Network grids
{
  ...
  return 1 if	abs($$gT[1] - $$extent{cellsizeX}) < 0.0001 * abs($$gT[1]) &&
