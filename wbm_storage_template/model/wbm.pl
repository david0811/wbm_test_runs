...
#      (C) University of New Hampshire and Water Systems Analysis Group 2008-2022
#
#######################################################################
...
use constant WBM_VERSION =>	'23.1.0';	# If this month is not listed in the notes above-
...
my ($idump,$errors,$dState,$test,$noRun,$spool_dir,$run_state_dir,$noStateID) =
   (  1,      0,      0,     0,     0,       0,           0,           0    );
my ($MT_file, $credits);
...
GetOptions(
	...
	'spoolDir=s'=>\$spool_dir, 'stateDir=s'=>\$run_state_dir, 'noStateID'=>\$noStateID) or exit();	usage() if $help;
...
foreach my $date (@$date_list)
{
  ...
  if ($runIO{Glaciers}) {
    ...
    $glVolume	= read_dateLayer($dataSet{glVolume},$date,$extent,$runIO{spool},
		{DATE_SEARCH_OPT=>{YR_UP=>1}, RESAMPLE=>0, PATCH_VALUE=>0})->lclip(0)*$glScale*0.917  # To water equivalent
			if defined $dataSet{glVolume};
...
Usage:
	$app_name [-h] [-v] [-bd|by|bm] [-test] [-noRun] [-rm] [-rmSpool] [-noOutput] [-noState] [-dState] [-t THREADS] [-tz THREAD_SIZE] [-sl] [-idump] [-saveDams] [-err] [-spoolDir SPOOL_DIR] [-stateDir STATE_DIR] [-noStateID] RUN_ID
